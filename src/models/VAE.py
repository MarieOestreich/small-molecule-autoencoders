...
class Autoencoder(pl.LightningModule):
    def __init__(self, rnn_type, hidden_size, num_layers, dropout, lr, batch_size,
                 ...
                 log_token_position_acc, tokenizer, max_seq_len, enumerated, p=0.3):
        ...
        self.fc_var = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, elbo, _, _ = self.model_run(batch, False)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss, elbo, output_dim, output = self.model_run(batch, False)

        mean_sim, eq_proc = self.compute_metrics(x_len, batch_size, output_dim, output, x, y, False, False)

        # logging
        self.log('val_ce', loss, on_epoch=True, sync_dist=True)
        self.log('val_loss', elbo, on_epoch=True, sync_dist=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss, elbo, output_dim, output = self.model_run(batch, self.save_latent)

        mean_sim, eq_proc = self.compute_metrics(x_len, batch_size, output_dim, output, x, y, True, self.save_latent)

        self.log('test_loss', loss)
        self.log('test_ce', elbo)
        self.log('test_eq_proc', eq_proc, on_epoch=True, sync_dist=True)
        self.log('test_mean_sim', mean_sim, on_epoch=True, sync_dist=True)
    ...
    def test_epoch_end(self, outputs):
        ...
        for key in self.mismatch_counter:
            if (self.char_counter[key] != 0):
                self.mismatch_freq[key] = self.mismatch_counter[key] / self.char_counter[key]
            else:
                self.mismatch_freq[key] = 0

        for key in self.mismatch_freq:
            self.log(key, self.mismatch_freq[key], sync_dist=True)
            self.log(key + '_occ', self.char_counter[key], sync_dist=True)
            self.log(key + '_mis_occ', self.mismatch_counter[key], sync_dist=True)
        ...
        if (self.save_latent):
            self.save_latent_as_csv()

        if (self.log_token_position_acc):
            self.log_token_pos_acc()
    ...
    def model_run(self, batch, save_latent):
        ...
        mu, log_var = self.fc_mu(encoder_states), self.fc_var(encoder_states)
        # sample z from q
        ...
        if (self.rnn_type == 'LSTM'):
            if (self.enumerated):
                output, hidden, cell = self.decoder(y_can, z)
            else:
                output, hidden, cell = self.decoder(x, z)
        else:
            if (self.enumerated):
                output, hidden = self.decoder(y_can, z)
            else:
                output, hidden = self.decoder(x, z)
        ...
        # kl
        kl = self.kl_divergence(z, mu, std, log_var, output_reshaped)
        # elbo
        elbo = self.p*kl + (1-self.p)*loss

        return loss, elbo, output_dim, output
    
    def kl_divergence(self, z, mu, std, log_var, pred):
        # --------------------------
        ...
        kl = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp()) / (z.shape[0] * pred.shape[1])
